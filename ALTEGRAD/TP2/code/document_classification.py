...
path_to_data = # add your absolute path here
...
all_terms = ### fill the gap ### hint: flatten 'terms_by_doc' (you may use a list comprehension)

# compute average number of terms
avg_len = sum(n_terms_per_doc)/len(n_terms_per_doc)

# unique terms
all_unique_terms = ### fill the gap ### hint: get unique elements of 'all_terms'
...
for counter,unique_term in enumerate(list(idf.keys())):
    ...
    # idf
    idf[unique_term] = ### fill the gap ### hint: use math.log10 and refer to the beginning of Section 2 in the handout
    if counter % 1e3 == 0:
...
collection_degrees = ### fill the gap ### hint: build a dict where the keys are the names of the nodes in the collection graph and the values are their unweighted degrees
...
for i, graph in enumerate(all_graphs):
    ...
    for term in list(set(terms_in_doc)):
        ...
        idf_term = idf[term]
        denominator = ### fill the gap ### hint: refer to the TF equation in the handout
